...
// Do NOT remove this file from your project! It is useful reference code that you and other users
// will want to refer to. Everything in this file will be stripped out by the linker if you don't
// call ImGui::ShowTestWindow(). During development, you can call ImGui::ShowTestWindow() in your
// code to learn about various features of ImGui. Removing this file from your project is hindering
// your access to documentation, likely leading you to poorer usage of the library.
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations" // warning : 'xx' is deprecated: The
                                                             // POSIX name for this item.. // for
                                                             // strdup used in demo code (so user
                                                             // can copy & paste the code)
...
#if(__GNUC__ >= 6)
#pragma GCC diagnostic ignored "-Wmisleading-indentation" // warning: this 'if' clause does not
...
void ImGui::ShowUserGuide() {
    ...
    ImGui::BulletText("While editing text:/n"
                      "- Hold SHIFT or use mouse to select text/n"
                      "- CTRL+Left/Right to word jump/n"
                      "- CTRL+A or double-click to select all/n"
                      "- CTRL+X,CTRL+C,CTRL+V clipboard/n"
                      "- CTRL+Z,CTRL+Y undo/redo/n"
                      "- ESCAPE to revert/n"
                      "- You can apply arithmetic operators +,*,/ on numerical values./n"
                      "  Use +- to subtract./n");
}
...
void ImGui::ShowTestWindow(bool *p_open) {
    ...
    // ImGui::PushItemWidth(ImGui::GetWindowWidth() * 0.65f);    // 2/3 of the space for widget and
    // 1/3 for labels
	...
	ImGui::TextWrapped("This window is being created by the ShowTestWindow() function. Please "
	                   "refer to the code for programming reference./n/nUser Guide:");
	ImGui::ShowUserGuide();
    }
	...
	if(ImGui::TreeNode("Trees")) {
		...
		ShowHelpMarker("This is a more standard looking tree with selectable nodes./nClick "
		               "to select, CTRL+Click to toggle, click on arrows or double-click "
		               "to open.");
		...
		static int selection_mask = (1 << 2); // Dumb representation of what may be
		                                      // user-side selection state. You may carry
		                                      // selection state inside or outside your
		                                      // objects in whatever format you see fit.
		int node_clicked = -1; // Temporary storage of what node we have clicked to process
		                       // selection at the end of the loop. May be a pointer to your
		                       // own node type, etc.
		ImGui::PushStyleVar(
		...
		for(int i = 0; i < 6; i++) {
			...
			if(ImGui::IsItemClicked())
			    node_clicked = i;
			if(node_open) {
			    ImGui::Text("Blah blah/nBlah Blah");
			    ImGui::TreePop();
			}
		    } else {
			// Leaf: The only reason we have a TreeNode at all is to allow selection of
			// the leaf. Otherwise we can use BulletText() or
		...
		if(node_clicked != -1) {
			...
			selection_mask ^= (1 << node_clicked); // CTRL+click to toggle
		         ...
		         // behavior you want, this commented bit preserve selection when clicking
	...
	if(ImGui::TreeNode("Bullets")) {
	    ...
	    ImGui::BulletText("Bullet point 2/nOn multiple lines");
	    ImGui::Bullet();
	...
	if(ImGui::TreeNode("Word Wrapping")) {
	    ...
	    ImGui::Text("The lazy dog is a good dog. This paragraph is made to fit within %.0f "
	                "pixels. Testing a 1 character word. The quick brown fox jumps over the "
	                "lazy dog.",
	                wrap_width);
	...
	if(ImGui::TreeNode("UTF-8 Text")) {
	    ...
	    // http://mplus-fonts.sourceforge.jp/mplus-outline-fonts/index-en.html) Most compiler
	    // appears to support UTF-8 in source code (with Visual Studio you need to save your
	    // file as 'UTF-8 without signature') However for the sake for maximum portability here
	    // we are *not* including raw UTF-8 character in this source file, instead we encode the
	    // string with hexadecimal constants. In your own application be reasonable and use
	    // UTF-8 in source or retrieve the data from file system! Note that characters values
	    // are preserved even if the font cannot be displayed, so you can safely copy & paste
	    // garbled characters into another application.
	    ImGui::TextWrapped("CJK text will only appears if the font was loaded with the "
	...
	if(ImGui::TreeNode("Images")) {
	    ImGui::TextWrapped("Below we are displaying the font texture (which is the only "
	                       "texture we have access to in this demo). Use the 'ImTextureID' "
	                       "type as storage to pass pointers or identifier to your own texture "
	                       "data. Hover the texture for a zoomed view!");
	...
	if(ImGui::TreeNode("Filtered Text Input")) {
	    ...
	    ImGui::SameLine();
	    ShowHelpMarker("Display all characters as '*'./nDisable clipboard cut and "
	                   "copy./nDisable logging./n");
	...
	if(ImGui::TreeNode("Multi-line Text Input")) {
	    ...
	    static char text[1024 * 16] =
	        "/*/n"
	        " The Pentium F00F bug, shorthand for F0 0F C7 C8,/n"
	        " the hexadecimal encoding of one offending instruction,/n"
	        " more formally, the invalid operand with locked CMPXCHG8B/n"
	        " instruction bug, is a design flaw in the majority of/n"
	        " Intel Pentium, Pentium MMX, and Pentium OverDrive/n"
	        " processors (all in the P5 microarchitecture)./n"
	        "*//n/n"
	        "label:/n"
	        "/tlock cmpxchg8b eax/n";
	...
	ImGui::Combo("combo scroll", &item2, items,
	             IM_ARRAYSIZE(items)); // Combo using proper array. You can also pass a callback
	                                   // to retrieve array value, no need to create/copy an
	                                   // array just for that.

	{
	    ...
	    ImGui::SameLine();
	    ShowHelpMarker("Hold SHIFT or use mouse to select text./n"
	                   "CTRL+Left/Right to word jump./n"
	                   "CTRL+A or double-click to select all./n"
	                   "CTRL+X,CTRL+C,CTRL+V clipboard./n"
	                   "CTRL+Z,CTRL+Y undo/redo./n"
	    ...
	    ImGui::SameLine();
	    ShowHelpMarker("You can apply arithmetic operators +,*,/ on numerical values./n  e.g. "
	                   "[ 100 ], input /'*2/', result becomes [ 200 ]/nUse +- to subtract./n");
...

	{
	    ...
	    ImGui::SameLine();
	    ShowHelpMarker("Click and drag to edit value./nHold SHIFT/ALT for faster/slower "
	                   "edit./nDouble-click or CTRL+click to input value.");
	...
	ImGui::SameLine();
	ShowHelpMarker("Click on the colored square to change edit mode./nCTRL+click on individual "
	               "component to input value./n");
	...
	ImGui::ListBox("listbox/n(single select)", &listbox_item_current, listbox_items,
	...
	if(ImGui::TreeNode("Vertical Sliders")) {
		...
		ImGui::VSliderFloat("##v", ImVec2(40, 160), &values[i], 0.0f, 1.0f, "%.2f/nsec");
		ImGui::PopStyleVar();
		ImGui::PopID();
	    }
	    ImGui::PopID();
	...
	if(ImGui::TreeNode("Child regions")) {
	    ...
	    ImGui::PopItemWidth();
	    ImGui::BeginChild("Sub1", ImVec2(ImGui::GetWindowContentRegionWidth() * 0.5f, 300),
	...
	if(ImGui::TreeNode("Widgets Width")) {
	    ...
	    ImGui::SameLine();
	    ShowHelpMarker(
	        "Half of available width./n(~ right-cursor_pos)/n(works within a column set)");
	...
	if(ImGui::TreeNode("Groups")) {
	    ...
	    ImGui::PlotHistogram("##values", values, IM_ARRAYSIZE(values), 0, NULL, 0.0f, 1.0f,
	                         size);
	    ...
	    ImGui::Button("LEVERAGE/nBUZZWORD", size);
	    ImGui::SameLine();
	...
	if(ImGui::TreeNode("Text Baseline Alignment")) {
	    ImGui::TextWrapped("(This is testing the vertical alignment that occurs on text to "
	                       "keep it at the same baseline as widgets. Lines only composed of "
	                       "text or \"small\" widgets fit in less vertical spaces than lines "
	                       "with normal widgets)");

	    ImGui::Text("One/nTwo/nThree");
	    ImGui::SameLine();
	    ImGui::Text("Hello/nWorld");
	    ImGui::SameLine();
	    ImGui::Text("Banana");
	    ...
	    ImGui::AlignFirstTextHeightToWidgets(); // If your line starts with text, call this to
	                                            // align it to upcoming widgets.
	    ...
	    ImGui::AlignFirstTextHeightToWidgets(); // Vertically align text node a bit lower so
	                                            // it'll be vertically centered with upcoming
	                                            // widget. Otherwise you can use SmallButton
	                                            // (smaller fit).
	    bool node_open = ImGui::TreeNode("Node##2"); // Common mistake to avoid: if we want to
	                                                 // SameLine after TreeNode we need to do it
	                                                 // before we add child content.
	    ImGui::SameLine(0.0f, spacing);
	...
	if(ImGui::TreeNode("Horizontal Scrolling")) {
	    ImGui::Bullet();
	    ImGui::TextWrapped("Horizontal scrolling for a window has to be enabled explicitly via "
	                       "the ImGuiWindowFlags_HorizontalScrollbar flag.");
	    ImGui::Bullet();
	    ImGui::TextWrapped("You may want to explicitly specify content width by calling "
		...
		// Button+SameLine. If you want to create your own time line for a real application
		// you may be better off manipulating the cursor position yourself, aka using
		// SetCursorPos/SetCursorScreenPos to position the widgets yourself. You may also
		// want to use the lower-level ImDrawList API)
		...
		for(int n = 0; n < num_buttons; n++) {
		    ...
		    const char *label =
		        (!(n % 15))
		            ...
		            : (!(n % 3))
		                  ? "Fizz"
		                  : (!(n % 5)) ? "Buzz" : (sprintf(num_buf, "%d", n), num_buf);
		    float hue = n * 0.05f;
	...
	if(ImGui::TreeNode("Clipping")) {
	    ...
	    ImGui::TextWrapped("On a per-widget basis we are occasionally clipping text CPU-side "
	                       "if it won't fit in its frame. Otherwise we are doing coarser "
	                       "clipping + passing a scissor rectangle to the renderer. The system "
	                       "is designed to try minimizing both execution and CPU/GPU rendering "
	                       "cost.");
	    ...
	    ImGui::GetWindowDrawList()->AddText(
	        ...
	        "Line 1 hello/nLine 2 clip me!", NULL, 0.0f, &clip_rect);
	    ImGui::TreePop();
	}
    }
	...
	if(ImGui::TreeNode("Popups")) {
	    ImGui::TextWrapped("When a popup is active, it inhibits interacting with windows that "
	    ...
	    // build a string using the "###" operator to preserve a constant ID with a variable
	    // label)
	    ...
	    ImGui::Separator();
	    // NB: As a quirk in this very specific example, we want to differentiate the parent of
	    // this menu from the parent of the various popup menus above. To do so we are encloding
	    // the items in a PushID()/PopID() block to make them two different menusets. If we
	    // don't, opening any popup above and hovering our menu here would open it. This is
	    // because once a menu is active, we allow to switch to a sibling menu by just hovering
	...
	if(ImGui::TreeNode("Modals")) {
	    ImGui::TextWrapped("Modal windows are like popups but the user cannot close them by "
	                       "clicking outside the window.");

	    if(ImGui::Button("Delete.."))
		ImGui::OpenPopup("Delete?");
	    if(ImGui::BeginPopupModal("Delete?", NULL, ImGuiWindowFlags_AlwaysAutoResize)) {
		ImGui::Text("All those beautiful files will be deleted./nThis operation cannot be "
		            "undone!/n/n");
		ImGui::Separator();
	...
	*/
	...
	ImGui::SameLine();
	ShowHelpMarker("NB: Tree node must be poped before ending the cell./nThere's no storage of "
	               "state per-cell.");
	if(node_open) {
	...
	if(ImGui::TreeNode("Tabbing")) {
	    ...
	    // ImGui::SameLine(); ShowHelperMarker("Use ImGui::PushAllowKeyboardFocus(bool)/nto
	    // disable tabbing through certain widgets.");
	...
	if(ImGui::TreeNode("Dragging")) {
		...
		draw_list->AddLine(
		    ImGui::CalcItemRectClosestPoint(ImGui::GetIO().MousePos, true, -2.0f),
		    ImGui::GetIO().MousePos, ImColor(ImGui::GetStyle().Colors[ImGuiCol_Button]),
		    4.0f);
	...
	if(ImGui::TreeNode("Keyboard & Mouse State")) {
	    ...
	    ImGui::Button("Hovering me sets the/nkeyboard capture flag");
	    if(ImGui::IsItemHovered())
		ImGui::CaptureKeyboardFromApp(true);
	    ImGui::SameLine();
	    ImGui::Button("Holding me clears the/nthe keyboard capture flag");
	    if(ImGui::IsItemActive())
	...
	if(ImGui::TreeNode("Mouse cursors")) {
	    ImGui::TextWrapped("Your application can render a different mouse cursor based on what "
	                       "ImGui::GetMouseCursor() returns. You can also set "
	                       "io.MouseDrawCursor to ask ImGui to render the cursor for you in "
	                       "software.");
...
void ImGui::ShowStyleEditor(ImGuiStyle *ref) {
    ...
    // it compares to the default style)
	...
	if(ImGui::Button("Copy Colors")) {
		...
		if(!output_only_modified ||
		   memcmp(&col, (ref ? &ref->Colors[i] : &default_style.Colors[i]),
		          sizeof(ImVec4)) != 0)
		    ImGui::LogText("style.Colors[ImGuiCol_%s]%*s= ImVec4(%.2ff, %.2ff, %.2ff, "
		                   "%.2ff);" IM_NEWLINE,
	...
	for(int i = 0; i < ImGuiCol_COUNT; i++) {
	    ...
	    if(memcmp(&style.Colors[i], (ref ? &ref->Colors[i] : &default_style.Colors[i]),
	              sizeof(ImVec4)) != 0) {
		ImGui::SameLine();
	...
	ImGui::SameLine();
	ShowHelpMarker("Tip: Load fonts with io.Fonts->AddFontFromFileTTF()/nbefore calling "
	...
	for(int i = 0; i < atlas->Fonts.Size; i++) {
		...
		ImGui::SameLine();
		ShowHelpMarker("Note than the default embedded font is NOT meant to be "
		               "scaled./n/nFont are currently rendered into bitmaps at a given "
		               "size at the time of building the atlas. You may oversample them to "
		               "get some flexibility with scaling. You can also render at multiple "
		               "sizes and select which one to use at runtime./n/n(Glimmer of hope: "
		               "the atlas system should hopefully be rewritten in the future to "
		...
		ImGui::Text("Texture surface: %d pixels (approx) ~ %dx%d",
		            font->MetricsTotalSurface, (int)sqrtf((float)font->MetricsTotalSurface),
		...
		if(ImGui::TreeNode("Glyphs", "Glyphs (%d)", font->Glyphs.Size)) {
		    ...
		    const ImFont::Glyph *glyph_fallback =
		        font->FallbackGlyph; // Forcefully/dodgily make FindGlyph() return NULL on
		                             // fallback, which isn't the default behavior.
			...
			if(count > 0 &&
			   ImGui::TreeNode((void *)(intptr_t)base, "U+%04X..U+%04X (%d %s)", base,
				...
				font->RenderChar(draw_list, cell_size.x, cell_p1,
				                 ImGui::GetColorU32(ImGuiCol_Text),
				                 (ImWchar)(base + n)); // We use ImFont::RenderChar
...
static void ShowExampleAppAutoResize(bool *p_open) {
    ...
    static int lines = 10;
    ImGui::Text("Window will resize every-frame to the size of its content./nNote that you "
                "probably don't want to query the window size to/noutput your content because that "
...
static void ShowExampleAppConstrainedResize(bool *p_open) {
    ...
    if(type == 5)
	ImGui::SetNextWindowSizeConstraints(ImVec2(0, 0), ImVec2(FLT_MAX, FLT_MAX),
	                                    CustomConstraints::Step, (void *)100); // Fixed Step
...
static void ShowExampleAppFixedOverlay(bool *p_open) {
    ...
    ImGui::Text("Simple overlay/non the top-left side of the screen.");
    ImGui::Separator();
...
// Read section "How can I have multiple widgets with the same label? Can I have widget without a
// label? (Yes). A primer on the purpose of labels/IDs." about ID.
static void ShowExampleAppManipulatingWindowTitle(bool *) {
    ...
    ImGui::Text(
        "This is window 1./nMy title is the same as window 2, but my identifier is unique.");
    ImGui::End();

    ImGui::SetNextWindowPos(ImVec2(100, 200), ImGuiSetCond_FirstUseEver);
    ImGui::Begin("Same title as another window##2");
    ImGui::Text(
        "This is window 2./nMy title is the same as window 1, but my identifier is unique.");
    ImGui::End();
...
static void ShowExampleAppCustomRendering(bool *p_open) {
	...
	for(int n = 0; n < 2; n++) {
	    ...
	    draw_list->AddCircle(ImVec2(x + sz * 0.5f, y + sz * 0.5f), sz * 0.5f, col32, 20,
	                         thickness);
	    x += sz + spacing;
	...
	draw_list->AddRectFilledMultiColor(ImVec2(x, y), ImVec2(x + sz, y + sz), ImColor(0, 0, 0),
	                                   ImColor(255, 0, 0), ImColor(255, 255, 0),
	                                   ImColor(0, 255, 0));
	...
	ImGui::Text("Left-click and drag to add lines,/nRight-click to undo");

	// Here we are using InvisibleButton() as a convenience to 1) advance the cursor and 2)
	// allows us to use IsItemHovered() However you can draw directly and poll mouse/keyboard by
	// yourself. You can manipulate the cursor using GetCursorPos() and SetCursorPos(). If you
	// only use the ImDrawList API, you can notify the owner window of its extends by using
	// SetCursorPos(max).
	...
	draw_list->AddRectFilledMultiColor(
	    ...
	    ImColor(50, 50, 50), ImColor(50, 50, 60), ImColor(60, 60, 70), ImColor(50, 50, 60));
...
// history. For the console example, here we are using a more C++ like approach of declaring a class
// to hold the data and the functions.
struct ExampleAppConsole {
	...
	ImGui::TextWrapped("This example implements a console with basic coloring, completion and "
	...
	for(int i = 0; i < Items.Size; i++) {
		...
		continue;
	    ImVec4 col = ImVec4(1.0f, 1.0f, 1.0f, 1.0f); // A better implementation may store a type
	                                                 // per-item. For the sample let's just
	                                                 // parse the text.
	...
	if(Stricmp(command_line, "CLEAR") == 0) {
		...
		AddLog("%3d: %s/n", i, History[i]);
	} else {
	    AddLog("Unknown command: '%s'/n", command_line);
	}
    }

    static int TextEditCallbackStub(ImGuiTextEditCallbackData *data) // In C++11 you are better off
                                                                     // using lambdas for this sort
                                                                     // of forwarding callbacks
    {
	...
	case ImGuiInputTextFlags_CallbackCompletion: {
		...
		// List matches
		AddLog("Possible matches:/n");
		for(int i = 0; i < candidates.Size; i++)
		    AddLog("- %s/n", candidates[i]);
	    }
...
//  my_log.AddLog("Hello %d world/n", 123);
//  my_log.Draw("title");
...
static void ShowExampleAppLog(bool *p_open) {
	...
	const char *random_words[] = {"system", "info",   "warning", "error",
	                              "fatal",  "notice", "log"};
	log.AddLog("[%s] Hello, time is %.1f, rand() %d/n",
...
static void ShowExampleAppPropertyEditor(bool *p_open) {
    ...
    ShowHelpMarker("This example shows how you may implement a property editor using two "
                   "columns./nAll objects/fields data are dummies here./nRemember that in many "
                   "simple cases, you can use ImGui::SameLine(xxx) to position/nyour cursor "
	...
	static void ShowDummyObject(const char *prefix, int uid) {
	    ...
	    ImGui::AlignFirstTextHeightToWidgets(); // Text and Tree nodes are less high than
	                                            // regular widgets, here we add vertical spacing
...
static void ShowExampleAppLongText(bool *p_open) {
    ...
    ImGui::Combo("Test type", &test_type,
                 ...
                 "manually/0Multiple calls to Text(), not clipped/0");
	...
	for(int i = 0; i < 1000; i++)
	    log.append("%i The quick brown fox jumps over the lazy dog/n", lines + i);
	lines += 1000;
    }
