...
// In this binding, ImTextureID is used to store a 'ID3D11ShaderResourceView*' texture identifier.
...
void ImGui_ImplDX11_RenderDrawLists(ImDrawData *draw_data) {
	...
	if(ctx->Map(g_pVertexConstantBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mapped_resource) !=
	   S_OK)
	    return;
    ...
    // Backup DX state that will be modified to restore it afterwards (unfortunately this is very
    // ugly looking and verbose. Close your eyes!)
	...
	ID3D11ClassInstance *     PSInstances[256],
	    *VSInstances[256]; // 256 is max according to PSSetShader documentation
	...
	for(int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++) {
		...
		const D3D11_RECT r = {(LONG)pcmd->ClipRect.x, (LONG)pcmd->ClipRect.y,
...
bool ImGui_ImplDX11_CreateDeviceObjects() {
    ...
    // given version of d3dcompiler_XX.dll (see D3DCOMPILER_DLL_A) If you would like to use this
    // DX11 sample code but remove this dependency you can:
	...
	if(g_pVertexShaderBlob == NULL) // NB: Pass ID3D10Blob* pErrorBlob to D3DCompile() to get
	...
	if(g_pd3dDevice->CreateVertexShader((DWORD *)g_pVertexShaderBlob->GetBufferPointer(),
	                                    g_pVertexShaderBlob->GetBufferSize(), NULL,
	...
	if(g_pPixelShaderBlob == NULL) // NB: Pass ID3D10Blob* pErrorBlob to D3DCompile() to get
	                               // error showing in (const
	                               // char*)pErrorBlob->GetBufferPointer(). Make sure to
	                               // Release() the blob!
	    return false;
	if(g_pd3dDevice->CreatePixelShader((DWORD *)g_pPixelShaderBlob->GetBufferPointer(),
	                                   g_pPixelShaderBlob->GetBufferSize(), NULL,
	                                   &g_pPixelShader) != S_OK)
	    return false;
    }
...
bool ImGui_ImplDX11_Init(void *hwnd, ID3D11Device *device, ID3D11DeviceContext *device_context) {
    ...
    io.RenderDrawListsFn = ImGui_ImplDX11_RenderDrawLists; // Alternatively you can set this to NULL
